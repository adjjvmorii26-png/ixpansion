...
Problem: dozens of wave organs + ops modules each with state files and
coherence_vitals(). Naive sequential import + vitals is slow and allocates.

Solution (this engine):
  1. Path-index     — discover wave*.py by filesystem only (no import)
  2. Bloom strip    — bit vector of which wave numbers exist (O(1) membership)
  3. Lazy vitals    — import+call only on demand or on full pulse; cache by mtime
  4. XOR fingerprint— fold all vitals JSON into a 256-bit organism DNA
  5. Delta gate     — skip downstream work if fingerprint unchanged

No external deps. Designed for lab dual-track (path-scoped, soft-fail).
"""
...
def build_index() -> Dict[str, Any]:
    """Filesystem-only index — zero imports."""
    paths = _wave_paths()
    by_num: Dict[int, str] = {}
    bloom = [0] * BLOOM_BITS
    for p in paths:
...
def fingerprint(vitals_list: List[Dict[str, Any]]) -> str:
    """XOR-fold SHA256 of each vitals blob → 64 hex chars (256-bit organism DNA)."""
    acc = bytearray(32)
