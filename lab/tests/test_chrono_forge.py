...
class TestChronoForgePort:
    ...
    def test_evolution_automations_are_advisory_and_consent_is_manual(self):
        ...
        assert all(item["path"] != "lab/recovery_shadow_red_cell.py" for item in data["projects"])
        assert all(item["path"] != "lab/recovery_manifest_loom.py" for item in data["projects"])
        assert all(item["path"] != "lab/recovery_answer_crucible.py" for item in data["projects"])
        ...
        assert all(data["projects"][index]["critical"] is False for index in (-5, -4, -3, -2, -1))
