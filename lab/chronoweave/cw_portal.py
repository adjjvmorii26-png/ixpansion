#!/usr/bin/env python3
"""Chronoweave portal — boot / timeline / causality / agents / ethics / sim."""
...
REPO = ROOT.parents[1]
ACTS = {
    "timeline": ROOT / "timeline_core" / "timeline_engine.py",
    "causality": ROOT / "causality" / "causal_engine.py",
    "agents": ROOT / "agents" / "agent_kernel.py",
    "ethics": ROOT / "ethics" / "ethics_temporal_gate.py",
...
def boot() -> dict:
    steps = []
    ok = True
    for name in ("ethics", "timeline", "causality", "agents"):
    ...
    try:
        ...
        with ledger.open("a") as f:
            f.write(json.dumps({"ts": datetime.now(timezone.utc).isoformat(), "type": "chronoweave_boot", "ref": f"ok={ok}"}) + "\n")
    except OSError:
        pass
...
def main() -> int:
    ...
    if act in ("help", "-h", "--help"):
        print(json.dumps({"acts": sorted(list(ACTS) + ["boot"]), "usage": "python cw_portal.py <act> [sim_name]"}, indent=2))
        return 0
    if act == "boot":
        print(json.dumps(boot(), indent=2))
        return 0
    if act == "sim":
        name = sys.argv[2] if len(sys.argv) > 2 else "branch"
