...
REPO = ROOT.parents[1]
ACTS = {
    "axioms": ROOT / "axioms" / "axiom_engine.py",
    "multilogic": ROOT / "multilogic" / "multilogic_engine.py",
    "agents": ROOT / "agents" / "agent_kernel.py",
    "forge": ROOT / "forge" / "forge_engine.py",
    "ethics": ROOT / "ethics" / "ethics_paradox_gate.py",
...
def boot() -> dict:
    steps = []
    ok = True
    for name in ("ethics", "axioms", "multilogic", "agents"):
    ...
    try:
        ...
        with ledger.open("a") as f:
            f.write(json.dumps({"ts": datetime.now(timezone.utc).isoformat(), "type": "paradox_forge_boot", "ref": f"ok={ok}"}) + "\n")
    except OSError:
        pass
...
def main() -> int:
    ...
    if act in ("help", "-h", "--help"):
        print(json.dumps({"acts": sorted(list(ACTS) + ["boot"]), "usage": "python pf_portal.py <act> [sim_name]"}, indent=2))
        return 0
    if act == "boot":
        print(json.dumps(boot(), indent=2))
        return 0
    if act == "sim":
        name = sys.argv[2] if len(sys.argv) > 2 else "superpose"
