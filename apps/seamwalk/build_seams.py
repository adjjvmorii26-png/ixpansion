#!/usr/bin/env python3
"""Build seam field from null orchard + graft + mood + helix."""
...
def main():
    ...
    if not helix:
        jrun([sys.executable, str(REPO / "lab" / "helix_bridge" / "probe.py")])
        if hp.exists(): helix = json.loads(hp.read_text())
    seams = []
    ...
    data = {"title": "SEAMWALK", "subtitle": "walk the rifts · do not fill them", "channel": "@CoodingLooop", "mood": mood.get("mood", "void"), "primary": mood.get("primary", "#00e5ff"), "secondary": mood.get("secondary", "#ff2bd6"), "n_seams": len(field), "field": field, "doctrine": "absence is structure", "ts": datetime.now(timezone.utc).isoformat()}
