...
# --- Wave 441: Wave Composition ---
    if path.startswith("/wave-composition") or path.startswith("/api/wave_composition"):
        from api.wave441_wave_composition import handler as h
        q = {} if "?" not in raw_path else dict(item.split("=", 1) for item in raw_path.split("?", 1)[1].split("&") if "=" in item)
        return h(q)

# --- Wave 442: Temporal Resonance ---
    if path.startswith("/temporal-resonance") or path.startswith("/api/temporal_resonance"):
        from api.wave442_temporal_resonance import handler as h
        q = {} if "?" not in raw_path else dict(item.split("=", 1) for item in raw_path.split("?", 1)[1].split("&") if "=" in item)
        return h(q)
...
# --- Wave 97: HEX Runtime ---
    if path.startswith("/hex-runtime") or path.startswith("/api/hex_runtime"):
        from api.wave97_hex_runtime import handler as h
        q = {} if "?" not in raw_path else dict(item.split("=", 1) for item in raw_path.split("?", 1)[1].split("&") if "=" in item)
        return h(q)

# --- Wave 98: HEX Cathedral Lacery ---
    if path.startswith("/hex-cathedral") or path.startswith("/api/hex_cathedral"):
        from api.wave98_hex_cathedral import handler as h
        q = {} if "?" not in raw_path else dict(item.split("=", 1) for item in raw_path.split("?", 1)[1].split("&") if "=" in item)
        return h(q)

# --- Wave 621: OmniRouter ---
    if path.startswith("/omnirouter") or path.startswith("/api/omnirouter"):
        from api.omnirouter import handler as h
        q = {} if "?" not in raw_path else dict(item.split("=", 1) for item in raw_path.split("?", 1)[1].split("&") if "=" in item)
        return h(q)

# --- Wave 622: Resilience Mesh ---
    if path.startswith("/resilience-mesh") or path.startswith("/api/resilience_mesh"):
        from api.wave622_resilience_mesh import handler as h
        q = {} if "?" not in raw_path else dict(item.split("=", 1) for item in raw_path.split("?", 1)[1].split("&") if "=" in item)
        return h(q)

# --- Wave 630: Performance Oracle ---
    if path.startswith("/performance-oracle") or path.startswith("/api/performance_oracle"):
...
# --- Wave 440: Linguistic Emergence ---
    ...
    # Wave-native alias: /api/resonance-chamber -> wave428_resonance_chamber
    if path.startswith("/api/"):
        alias_name = path[len("/api/"):].split("?")[0].strip("/").split("/")[0].replace("-", "_")
        if alias_name and not alias_name.startswith("wave") and "." not in alias_name:
            api_dir = Path(__file__).resolve().parent
            candidates = sorted(api_dir.glob(f"wave*_{alias_name}.py"))
            if candidates:
                mod = candidates[-1].stem
                q = {} if "?" not in raw_path else dict(
                    item.split("=", 1) for item in raw_path.split("?", 1)[1].split("&") if "=" in item)
                result, _status = api_server.call_handler(mod, q)
                return result
...
def handler(request) -> dict:
    ...
    # Organism Status — Control Center
    if path.startswith("/organism-status") or path.startswith("/api/organism_status"):
        from api.organism_status import handler as h
        q = {} if "?" not in raw_path else dict(item.split("=", 1) for item in raw_path.split("?", 1)[1].split("&") if "=" in item)
        return h(q)
    if path == "/organism-status" or path == "/organism_status":
        from api.organism_status import handler as h
        q = {} if "?" not in raw_path else dict(item.split("=", 1) for item in raw_path.split("?", 1)[1].split("&") if "=" in item)
        return h(q)
    ...
    # Wave 431 — Homestead
    if path.startswith("/homestead") or path.startswith("/api/homestead"):
        from api.wave431_homestead import handler as h
        q = {} if "?" not in raw_path else dict(item.split("=", 1) for item in raw_path.split("?", 1)[1].split("&") if "=" in item)
        return h(q)
    if path == "/homestead":
        from api.wave431_homestead import handler as h
...
# --- Wave 639: Echo Chamber Breaker ---
    if path.startswith("/echo-breaker") or path.startswith("/api/echo_breaker"):
        from api.wave639_echo_breaker import handler as h
        q = {} if "?" not in raw_path else dict(item.split("=", 1) for item in raw_path.split("?", 1)[1].split("&") if "=" in item)
        return h(q)

# --- Wave 640: Dependency Resolver ---
    if path.startswith("/dependency-resolver") or path.startswith("/api/dependency_resolver"):
...
# --- Wave 696: Harmonic Resonance Oracle ---
    if path.startswith("/harmonic-resonance") or path.startswith("/api/harmonic_resonance"):
...
# --- Wave 699: Entropy Heatmap ---
    if path.startswith("/entropy-heatmap") or path.startswith("/api/entropy_heatmap"):
