"""Wave 999 — recurse_spark_halo.

Recursive evolution of dream_residue_collector — deepens the spark layer through self-referential recursion.

Parent: dream_residue_collector
Lineage: recursive
"""
...
DATA_FILE = ROOT / "data" / "wave999_recurse_spark_halo.json"
WAVE = 999
NAME = "recurse_spark_halo"
PARENT = "dream_residue_collector"
...
def handler(req: dict) -> dict:
    ...
    if action == "spark_action":
        state.setdefault("actions_taken", []).append({"action": "spark_action", "at": datetime.datetime.now(datetime.UTC).isoformat()})
        _save(state)
        return {"wave": WAVE, "name": NAME, "action": "spark_action", "ok": True, "count": len(state.get("actions_taken", []))}
...
def resonates_with() -> list:
    return ["harmony_report", "mutation_engine", "dream_residue_collector"]
