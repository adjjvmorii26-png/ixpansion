...
def build(records: List[Dict[str, Any]] | None = None) -> Dict[str, Any]:
    ...
    divergence = sorted(
        ({"node": rid, "children": len(children[rid])} for rid in ids if len(children[rid]) > 1),
        key=lambda x: x["node"],
    )
    convergence = []
    for rid in ids:
        ...
        if len(incoming) > 1:
            convergence.append({"node": rid, "parents": sorted(incoming)})
    signatures = {}
...
def handler(payload: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    if payload.get("action", "status") == "status":
        return {"status": "experimental", "wave": 906, "name": "evolution_map", "purpose": "descriptive lineage structure"}
